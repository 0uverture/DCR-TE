...
\pagebreak
	...
	The three primary components of the proposed solution are as follows: the workflow, which will be described with DCR, Intel SGX and method for achieving distributed consensus.
		...
		To ensure that keys are only distributed to trusted users, Intel offers Attestation Services (IAS) as a way of key distribution.
			...
			\begin{figure}[ht]
				\includegraphics{figures/remote-attestation-flow.png}
				\caption{Flow of remote attestation.}
				\label{fig:remote-attestation}
			\end{figure}

			The specific details of the remote attestation is then up to the API exposed by the ISV, but is outlined in figure \ref{fig:remote-attestation}\footnote{Taken from \url{https://software.intel.com/en-us/articles/intel-software-guard-extensions-remote-attestation-end-to-end-example}}.
